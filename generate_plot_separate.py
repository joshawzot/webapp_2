...
#separate
def plot_histogram_with_fitting(data, table_names, figsize=(15, 10)):
    ...
    for table_index, table_data in enumerate(data):
        # Check if there are any subgroups with data to avoid errors
        if any(len(subgroup) > 0 for subgroup in table_data):
            # Determine the global range for consistent binning
            global_min = min(min(subgroup) for subgroup in table_data if len(subgroup) > 0)
            global_max = max(max(subgroup) for subgroup in table_data if len(subgroup) > 0)
            bins = np.linspace(global_min, global_max, 101)  # 100 bins

            cmap = cm.get_cmap('viridis', len(table_data))
            colors = [cmap(i / len(table_data)) for i in range(len(table_data))]
            ...
            for state_index, subgroup in enumerate(table_data):
                ...
                if len(subgroup) == 0:
                    continue  # Skip processing for empty subgroups
                ...
                total = sum(counts)  # Calculate total counts for use in fitting curve

                # Percentage plot
                weights = np.ones_like(subgroup) / len(subgroup)
                percentage, bins, patches = ax_percentage.hist(subgroup, bins=bins, weights=weights, color=color, alpha=0.6, label=f'State {state_index + 1} (Percentage)')
                ...
                # Calculate the fitting curve based on the distribution parameters
            ...
            # Save and close the plots, append encoded data
...
def generate_plot_separate(table_names, database_name, form_data):
    ...
    for table_name in table_names:
        groups, stats, _, _, selected_groups= get_group_data_new(table_name, selected_groups, database_name, sub_array_size)
        encoded_plot1, encoded_plot2, table_fitting_params = plot_histogram_with_fitting([groups], [table_name])
